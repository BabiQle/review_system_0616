...
const APP_VERSION = "v1.1 on 2026.6.18";
...
function useHasReviewed() {
  ...
  const checkReview = useCallback(async () => {
    ...
    try {
      ...
      const REVIEWER_FIELD = 'from_user';

      // ✅ 修复：使用日期范围比较，而不是 year()/month() 函数
      const filter = `${REVIEWER_FIELD} = "${profile.id}" && created >= "${startStr}" && created < "${endStr}"`;
      console.log('🔍 查询本月评价过滤条件:', filter);

      const result = await pb.collection(COLLECTION_NAME).getList(1, 1, {
        filter: filter,
      });

      console.log('📋 查询结果:', result.items);
      setState({ hasReviewed: result.items.length > 0, loading: false });
    } catch (error) {
      console.error('❌ 检查评价记录失败:', error);
      setState({ hasReviewed: false, loading: false });
    }
  }, [profile]);
...
const NavItems = memo(function NavItems({
  ...
  return (
    <nav className="flex flex-col gap-1 px-3 py-2">
      {items.map((item) => {
        // 显示条件：路径为 /review，用户有权限，加载完成，且尚未评价
        const showLabel =
          item.path === '/review' &&
          ...
          >
            ...
            <div className="flex items-center gap-2 min-w-0">
              ...
              {showLabel && (
                <Badge
                  variant="destructive"
                  className="text-[10px] leading-4 px-1.5 py-0 rounded-sm font-medium whitespace-nowrap shrink-0"
                >
                  未填写
                </Badge>
              )}
