...
export default function MyStatsPage() {
  ...
  // 如果用户不能发送评价，强制切换到 received 标签
  useEffect(() => {
    if (!canSend && activeTab === 'sent') {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const cycleReviews = result.items.filter((r) => {
        ...
        const parsedReviews = cycleReviews.map((r) => {
        ...
        const sentArray = Array.from(sentMap.values())
          .sort((a, b) => b.avgScore - a.avgScore)
          .map((s, i) => ({ ...s, rank: i + 1 }));
        const receivedArray = Array.from(receivedMap.values())
          .sort((a, b) => b.avgScore - a.avgScore)
          .map((s, i) => ({ ...s, rank: i + 1 }));

        setSentStats(sentArray);
        setReceivedStats(receivedArray);
  ...
  const getDisplayUserName = (stat: PersonStat, statsList: PersonStat[]) => {
    if (activeTab === 'sent') return stat.userName;
    const sortedList = [...statsList].sort((a, b) => b.avgScore - a.avgScore);
    const idx = sortedList.findIndex(item => item.userId === stat.userId);
    const rankNum = pokerRanks[idx] || `${idx + 1}`;
    const suitIcon = getSuit(stat.avgScore);
    return `${rankNum} ${suitIcon}`;
  };

  const getDimensionAverages = (stats: PersonStat[]) => {
    const dimAverages: Record<string, number> = {};
    DIMENSIONS.forEach(dim => {
      const scores = stats.map(s => s.dimensionScores[dim] || 0);
      const sum = scores.reduce((a, b) => a + b, 0);
      dimAverages[dim] = stats.length > 0 ? sum / stats.length : 0;
    });
    return dimAverages;
  };

  const currentStats = activeTab === 'sent' ? sentStats : receivedStats;
  const dimAverages = getDimensionAverages(currentStats);
  ...
  // 准备柱状图数据 - 垂直柱状图格式
  const rankingData = currentStats.slice(0, 10).map(s => {
    if (activeTab === 'sent') {
      const displayName = s.userName.length > 8 ? s.userName.slice(0, 6) + '..' : s.userName;
      return {
        name: displayName,
        fullName: s.userName,
    ...
    const sortedList = [...currentStats].sort((a, b) => b.avgScore - a.avgScore);
    const idx = sortedList.findIndex(item => item.userId === s.userId);
    const rankNum = pokerRanks[idx] || `${idx + 1}`;
    const suitIcon = getSuit(s.avgScore);
    return {
      name: `${rankNum} ${suitIcon}`,
      fullName: `${rankNum} ${suitIcon}`,
      score: Number(s.avgScore.toFixed(2)),
    };
  });
  ...
  const getTrendIcon = (rank: number, stats: PersonStat[]) => {
    ...
    if (rank >= stats.length - 1 && stats.length > 3) return <ArrowDown className="h-3.5 w-3.5 text-red-500" />;
  ...
  return (
    <div className="h-screen bg-slate-50 flex flex-col overflow-hidden select-none">
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between flex-wrap gap-4">
                ...
                <SelectContent>
                  {cycles.map((c) => (
                    <SelectItem key={c.id} value={c.id}>
                      {c.name}
                      {getCurrentMonthBadge(c) && <Badge variant="secondary" className="ml-2 text-xs">本月</Badge>}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <div className="flex bg-slate-100 rounded-lg p-1 gap-1">
            ...
            >
              ...
              收到的评价
              <Badge className={`${activeTab === 'received' ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-600'} text-xs`}>
            ...
            {canSend && (
              ...
              >
                ...
                发出的评价
                <Badge className={`${activeTab === 'sent' ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-600'} text-xs`}>
                  {sentStats.length}
                </Badge>
              </button>
            )}
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <XAxis
                        dataKey="name"
                        tick={{
                          angle: activeTab === 'sent' ? -20 : 0,
                          textAnchor: activeTab === 'sent' ? 'end' : 'middle',
                          fontSize: 11,
                          fill: '#334155'
                        }}
                        interval={0}
                      ...
                      <YAxis
                        ...
                        axisLine={false}
                        tickLine={false}
                        label={{ value: '平均分', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fill: '#64748b', fontSize: 12 } }}
                      />
                  ...
                  <div className="space-y-3">
                    <AnimatePresence>
                      {currentStats.map((stat, index) => {
                        const displayName = getDisplayUserName(stat, currentStats);
                        return (
                          <motion.div
                            key={stat.userId}
                          ...
                          >
                            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                              <div className="flex items-center gap-3">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shadow-sm ${getRankBadgeStyle(stat.rank || index + 1)}`}>
                                  {stat.rank || index + 1}
                                </div>
                                <div className="flex items-center gap-2">
                                  <span className="text-base font-semibold text-slate-900">{displayName}</span>
                                  {getTrendIcon(stat.rank || index + 1, currentStats)}
                                </div>
                              </div>
...
function OverviewCard({ title, value, suffix, icon, color, desc }: {
  title: string;
  value: string | number;
  suffix: string;
