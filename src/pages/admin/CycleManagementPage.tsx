...
// ===== 周期卡片（memo 避免无谓重绘） =====
const CycleCard = memo(function CycleCard({
  cycle,
  updating,
  onToggle,
  index,
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm transition-all select-none cursor-default"
    >
...
export default function CycleManagementPage() {
  ...
  const autoCreateMissingCycles = useCallback(async (currentCycles: MonthCycle[]) => {
    ...
    try {
      ...
      for (let i = 1; i <= 1; i++) {
        ...
        if (currentCycles.some(c => c.name === cycleName)) continue;
  ...
  // 切换周期状态 —— 通过 ref 读写最新数据，依赖为空，引用永不变化
  const toggleCycle = useCallback(
    (cycleId: string) => {
      const currentCycles = cyclesRef.current;
      const currentUpdatingId = updatingIdRef.current;
    ...
    <div className="flex-1 flex flex-col overflow-hidden select-none">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-5 select-none"
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <ul className="space-y-1.5 text-sm text-slate-700">
                <li className="flex items-start gap-2"><span className="text-blue-500 mt-1">•</span><span>周期按自然月自动划分，每月 1 日至月底。</span></li>
                <li className="flex items-start gap-2"><span className="text-blue-500 mt-1">•</span><span>系统会在管理员首次访问时自动创建下个月的评价周期，无需手动操作。</span></li>
                <li className="flex items-start gap-2"><span className="text-blue-500 mt-1">•</span><span>当月周期默认开启；历史月份默认关闭，可手动开启补填。</span></li>
                <li className="flex items-start gap-2"><span className="text-blue-500 mt-1">•</span><span>关闭后成员无法在该月填写新评价，已有数据不受影响。</span></li>
              </ul>
            </div>
          </div>
        </motion.div>
